...
//业务日期表
type DocBaseurl struct {
	Id int64 `orm:"column(id);pk;auto"`
	//业务日期
	DocnameId int64 `orm:"column(docname_id);size(20)"`
	//@todo TODO 待处理 DONE 处理完成
	Baseurl string `orm:"column(baseurl);size(255);" form:"Baseurl"`
...
func (n *DocBaseurl) TableName() string {
	return beego.AppConfig.String("doc_baseurl_table")
}

//验证用户信息
func checkDocBaseurl(u *DocBaseurl) (err error) {
...
func init() {
	orm.RegisterModel(new(DocBaseurl))
}

//get node list
func GetDocBaseurllist(page int64, page_size int64, sort string) (nodes []orm.Params, count int64) {
	o := orm.NewOrm()
	node := new(DocBaseurl)
...
func GetDocBaseurlById(nid int64) (DocBaseurl, error) {
	o := orm.NewOrm()
	node := DocBaseurl{Id: nid}
	err := o.Read(&node)
	if err != nil {
		return node, err
	}
...
//添加用户
func AddDocBaseurl(n *DocBaseurl) (int64, error) {
	if err := checkDocBaseurl(n); err != nil {
		return 0, err
	}
	o := orm.NewOrm()
	node := new(DocBaseurl)
...
//更新用户
func UpdateDocBaseurl(n *DocBaseurl) (int64, error) {
	if err := checkDocBaseurl(n); err != nil {
		return 0, err
	}
	o := orm.NewOrm()
	...
	// }
	var table DocBaseurl
	num, err := o.QueryTable(table).Filter("Id", n.Id).Update(node)
	return num, err
}

func DelDocBaseurlById(Id int64) (int64, error) {
	o := orm.NewOrm()
	status, err := o.Delete(&DocBaseurl{Id: Id})
	return status, err
}

func GetDocBaseurllistByGroupid(Groupid int64) (nodes []orm.Params, count int64) {
	o := orm.NewOrm()
	node := new(DocBaseurl)
...
func GetDocBaseurlTree(pid int64, level int64) ([]orm.Params, error) {
	o := orm.NewOrm()
	node := new(DocBaseurl)
...
// no records exist
func GetAllDocBaseurls(query map[string]string, fields []string, sortby []string, order []string,
