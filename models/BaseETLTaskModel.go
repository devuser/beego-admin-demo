...
//BaseETLTask ETL任务
type BaseETLTask struct {
	Id int64 `orm:"column(task_id);pk;auto"`
	//ETL描述索引
	TaskDescId *BaseETLTaskDesc `orm:"column(taskdesc_id);rel(fk)"`
	//业务日期
...
func (n *BaseETLTask) TableName() string {
	return beego.AppConfig.String("base_etl_task_table")
}

//验证用户信息
func checkBaseETLTask(u *BaseETLTask) (err error) {
...
func init() {
	orm.RegisterModel(new(BaseETLTask))
}

//get node list
func GetBaseETLTasklist(page int64, page_size int64, sort string) (nodes []orm.Params, count int64) {
	o := orm.NewOrm()
	node := new(BaseETLTask)
...
func GetBaseETLTaskById(nid int64) (BaseETLTask, error) {
	o := orm.NewOrm()
	node := BaseETLTask{Id: nid}
	err := o.Read(&node)
	if err != nil {
		return node, err
	}
...
//添加用户
func AddBaseETLTask(n *BaseETLTask) (int64, error) {
	if err := checkBaseETLTask(n); err != nil {
		return 0, err
	}
	o := orm.NewOrm()
	node := new(BaseETLTask)
...
//更新用户
func UpdateBaseETLTaskById(n *BaseETLTask) (int64, error) {
	if err := checkBaseETLTask(n); err != nil {
		return 0, err
	}
	o := orm.NewOrm()
	...
	// }
	var table BaseETLTask
	num, err := o.QueryTable(table).Filter("Id", n.Id).Update(node)
	return num, err
}

func DelBaseETLTaskById(Id int64) (int64, error) {
	o := orm.NewOrm()
	status, err := o.Delete(&BaseETLTask{Id: Id})
	return status, err
}

func GetBaseETLTasklistByGroupid(Groupid int64) (nodes []orm.Params, count int64) {
	o := orm.NewOrm()
	node := new(BaseETLTask)
...
func GetBaseETLTaskTree(pid int64, level int64) ([]orm.Params, error) {
	o := orm.NewOrm()
	node := new(BaseETLTask)
...
// no records exist
func GetAllBaseETLTask(query map[string]string, fields []string, sortby []string, order []string,
