...
//构造字段
// DPSzhminx 账户明细 @todo 待核实
type DPSzhminx struct {
...
func init() {
	orm.RegisterModel(new(DPSzhminx))
}

// AddDPSzhminx insert a new DPSzhminx into database and returns
// last inserted Id on success.
func AddDPSzhminx(m *DPSzhminx) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetDPSzhminxById retrieves DPSzhminx by Id. Returns error if
// Id doesn't exist
func GetDPSzhminxById(id int64) (v *DPSzhminx, err error) {
	o := orm.NewOrm()
	v = &DPSzhminx{Id: id}
...
func GetDPSzhminxList(page int64, page_size int64, sort string) (nodes []orm.Params, count int64) {
	o := orm.NewOrm()
	node := new(DPSzhminx)
...
// GetAllDPSzhminx retrieves all DPSzhminx matches certain condition. Returns empty list if
// no records exist
func GetAllDPSzhminxs(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(DPSzhminx))
	// query k=v
	for k, v := range query {
...
// UpdateDPSzhminx updates DPSzhminx by Id and returns error if
// the record to be updated doesn't exist
func UpdateDPSzhminxById(m *DPSzhminx) (err error) {
	o := orm.NewOrm()
	v := DPSzhminx{Id: m.Id}
...
// DeleteDPSzhminx deletes DPSzhminx by Id and returns error if
// the record to be deleted doesn't exist
func DelDPSzhminxById(id int64) (err error) {
	o := orm.NewOrm()
	v := DPSzhminx{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&DPSzhminx{Id: id}); err == nil {
