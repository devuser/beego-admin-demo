...
import (
	...
	m "github.com/devuser/beego-admin-demo/models"
)
...
func (this *MainController) Index() {
	...
	fmt.Printf("MainController.Index %q\n", userinfo)
	if userinfo == nil {
...
func (this *MainController) Login() {
	...
	if isajax == "1" {
		...
		fmt.Printf("MainController.username %q\n", username)
		fmt.Printf("MainController.password %q\n", password)
		user, err := CheckLogin(username, password)
		if err == nil {
			fmt.Printf("After CheckLogin %q\n", user)
			this.SetSession("userinfo", user)
			accesslist, _ := GetAccessList(user.Id)
			fmt.Printf("accesslist %q\n", accesslist)
