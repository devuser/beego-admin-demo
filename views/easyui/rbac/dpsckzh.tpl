...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        onAfterEdit:function(index, data, changes){
            ...
            changes.Id = data.Id;
            vac.ajax(URL+'/UpdateDPSCkzh', changes, 'POST', function(r){
                if(!r.status){
                    vac.alert(r.info);
                }else{
    ...
    $("#dialog2").dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                ...
                vac.ajax(URL+'/UpdateDPSCkzh', {Id:selectedRow.Id,Password:password}, 'post', function(r){
                    if(r.status){
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DeleteDPSCkzh', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
...
<div id="dialog" title="添加存款账户" style="width:400px;height:400px;">
    <div style="padding:20px 20px 40px 80px;" >
        <form id="form1" method="post">
            <table>
                <tr>
                    <td>存款账户名称：</td>
                    <td><input name="Dpscklx" class="easyui-validatebox" required="true"/></td>
                </tr>
                <tr>
                    <td>描述：</td>
