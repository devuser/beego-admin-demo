...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        columns:[[
            ...
            {field:'Kehuhaoo',title:'客户号',width:50,sortable:true},
            {field:'Kehuzhwm',title:'客户中文名称',width:100,sortable:true},
            {field:'Guishjig',title:'归属机构',width:100,sortable:true},
            {field:'Chjianjg',title:'创建机构',width:100,sortable:true},
            {field:'Hostgroupcode',title:'所属集团',width:100,sortable:true},

            {field:'KeHuHao',title:'客户号',width:100,sortable:true},
            {field:'Kehuzhwm',title:'客户中文名称',width:100,sortable:true},
            {field:'KehuLeiXing',title:'客户类型编码',width:100,sortable:true},
            {field:'KehuFenLei',title:'客户分类编码',width:100,sortable:true},


            {field:'QiYeKeHuFaRenXingZhi',title:'企业客户法人性质',width:100,sortable:true},
            {field:'Qiyeguim',title:'企业分类（大中小微）',width:100,sortable:true},

            {field:'FaRenLeiBie',title:'法人类别',width:100,sortable:true},
            {field:'Kehuleix',title:'是否同业',width:100,sortable:true},
            {field:'TongYeKeHuFaRenXingZhi',title:'同业客户法人性质',width:100,sortable:true},

            {field:'FengCunFlag',title:'是否封存',width:100,sortable:true},
            {field:'ChuangJianRiQI',title:'创建日期',width:100,sortable:true},

            {field:'ZhengJianLX',title:'证件类型',width:100,sortable:true},
            {field:'ZuZhiJiGouDaiMaZ',title:'组织机构代码证',width:100,sortable:true},
            {field:'KeHuBingBiaoFlag',title:'客户并表标识',width:100,sortable:true},
            {field:'Kehuztai',title:'客户状态',width:100,sortable:true},

              {field:'TongYeKehuLX',title:'同业客户类型_使用场景待核实',width:100,sortable:true},
              {field:'ZhengZhaoZhongLei',title:'证照种类编码',width:100,sortable:true},
            ...
            {field:'CreateAt',title:'创建时间',width:100,align:'center',
                formatter:function(value,row,index){
                    if(value) return phpjs.date("Y-m-d H:i:s",phpjs.strtotime(value));
                    return value;
                }
            },
            {field:'UpdateAt',title:'修改时间',width:100,align:'center',
                formatter:function(value,row,index){
                    if(value) return phpjs.date("Y-m-d H:i:s",phpjs.strtotime(value));
                    return value;
                }
        ...
        onAfterEdit:function(index, data, changes){
            ...
            changes.Id = data.Id;
            vac.ajax(URL+'/UpdateBasekehu', changes, 'POST', function(r){
                if(!r.status){
                    vac.alert(r.info);
                }else{
    ...
    $("#dialog2").dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                ...
                vac.ajax(URL+'/UpdateBasekehu', {Id:selectedRow.Id,Password:password}, 'post', function(r){
                    if(r.status){
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DelBasekehu', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
...
<div id="dialog" title="添加存款客户列表" style="width:400px;height:400px;">
