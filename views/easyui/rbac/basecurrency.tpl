...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        columns:[[
            ...
            {field:'Huobdaih',title:'货币代号',width:50,sortable:true},
            {field:'Bzbiemng',title:'货币英文名称',width:100,sortable:true},
            {field:'Hbmcheng',title:'货币中文名称',width:100,sortable:true},
            {field:'Huobzmsx',title:'货币字母缩写',width:100,sortable:true},
            {field:'Huobfhao',title:'货币符号',width:100,sortable:true},
            {field:'Guobiedm',title:'国别代码',width:100,sortable:true},
        ...
        onAfterEdit:function(index, data, changes){
            ...
            changes.Id = data.Id;
            vac.ajax(URL+'/UpdateBasecurrency', changes, 'POST', function(r){
                if(!r.status){
                    vac.alert(r.info);
                }else{
    ...
    $("#dialog2").dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                ...
                vac.ajax(URL+'/UpdateBasecurrency', {Id:selectedRow.Id,Password:password}, 'post', function(r){
                    if(r.status){
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DelBasecurrency', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
...
<div id="dialog" title="添加货币代码" style="width:400px;height:400px;">
    <div style="padding:20px 20px 40px 80px;" >
        <form id="form1" method="post">
            <table>
                <tr>
                    <td>货币代码名称：</td>
                    <td><input name="BaseCurrency" class="easyui-validatebox" required="true"/></td>
                </tr>
                <tr>
                    <td>描述：</td>
