...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        columns:[[
            ...
            {field:'Bizdate',title:'业务日期',width:100,align:'center',
                formatter:function(value,row,index){
                    if(value) return phpjs.date("Y-m-d",phpjs.strtotime(value));
                    return value;
                }
            },
            {field:'State',title:'完成状态',width:50,align:'center',
        ...
        onAfterEdit:function(index, data, changes){
            ...
            changes.Id = data.Id;
            vac.ajax(URL+'/UpdateBaseBizdate', changes, 'POST', function(r){
                if(!r.status){
                    vac.alert(r.info);
                }else{
    ...
    $("#dialog2").dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                ...
                vac.ajax(URL+'/UpdateBaseBizdate', {Id:selectedRow.Id,Password:password}, 'post', function(r){
                    if(r.status){
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DelBaseBizdate', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
    ...
    <div style="padding:20px 20px 40px 80px;" >
        <form id="form1" method="post">
            <table>
                <tr>
                    <td>文档名称：</td>
                    <td><input name="BaseBizdate" class="easyui-validatebox" required="true"/></td>
                </tr>
                <tr>
                    <td>描述：</td>
