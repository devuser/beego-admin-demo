...
$(function(){
    ...
    $("#datagrid").datagrid({
        ...
        columns:[[
            ...
            {field:'Groupname',title:'报表组名称',width:100,align:'center',sortable:true,editor:'text'},
            {field:'Description',title:'报表组描述',width:100,align:'left',sortable:true,editor:'text'},
        ...
        onAfterEdit:function(index, data, changes){
            ...
            changes.Id = data.Id;
            vac.ajax(URL+'/UpdateDocgroup', changes, 'POST', function(r){
                if(!r.status){
                    vac.alert(r.info);
                }else{
    ...
    $("#dialog2").dialog({
        ...
        buttons:[{
            ...
            handler:function(){
                ...
                vac.ajax(URL+'/UpdateDocgroup', {Id:selectedRow.Id,Password:password}, 'post', function(r){
                    if(r.status){
...
function delrow(){
    $.messager.confirm('Confirm','你确定要删除?',function(r){
        if (r){
            ...
            vac.ajax(URL+'/DelDocgroup', {Id:row.Id}, 'POST', function(r){
                if(r.status){
                    $("#datagrid").datagrid('reload');
                }else{
                    vac.alert(r.info);
    ...
    <div style="padding:20px 20px 40px 80px;" >
        <form id="form1" method="post">
            <table>
                <tr>
                    <td>文档名称：</td>
                    <td><input name="Docgroup" class="easyui-validatebox" required="true"/></td>
                </tr>
                <tr>
                    <td>描述：</td>
